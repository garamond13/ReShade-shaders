...
void XeGTAO_MainPass(uint2 pixCoord, float2 localNoise, Texture2D sourceViewspaceDepth, SamplerState depthSampler, RWTexture2D<unorm float2> outWorkingAOTermAndEdges)
{
...
RWTexture2D<unorm float> outputTexture
#else
...
#if XE_GTAO_FINAL_APPLY
RWTexture2D<unorm float> final_output : register(u0);
#else
...
void prefilter_depths16x16_cs(uint3 dtid : SV_DispatchThreadID, uint2 gtid : SV_GroupThreadID)
{
	// tex0 = linearized depth
	// smp = g_samplerPointClamp
	XeGTAO_PrefilterDepths16x16(dtid.xy, gtid, tex0, smp, out_working_depth_mip0, out_working_depth_mip1, out_working_depth_mip2, out_working_depth_mip3, out_working_depth_mip4);
}

[numthreads(XE_GTAO_NUMTHREADS_X, XE_GTAO_NUMTHREADS_Y, 1)]
void main_pass_cs(uint3 dtid : SV_DispatchThreadID)
{
	// tex0 = g_srcWorkingDepth
	// smp = g_samplerPointClamp
	XeGTAO_MainPass(dtid.xy, SpatioTemporalNoise(dtid.xy, 0), tex0, smp, ao_term_and_edges);
}

[numthreads(XE_GTAO_NUMTHREADS_X, XE_GTAO_NUMTHREADS_Y, 1)]
void denoise_pass_cs(uint3 dtid : SV_DispatchThreadID)
{
	// tex0 = g_srcWorkingAOTerm and g_srcWorkingEdges, packed
	// smp = g_samplerPointClamp
	const uint2 pix_coord_base = dtid.xy * uint2(2, 1); // we're computing 2 horizontal pixels at a time (performance optimization)
	XeGTAO_Denoise(pix_coord_base, tex0, smp, final_output);
}

// Expects fullscreen triangle VS.
float4 copy_ps(float4 pos : SV_Position) : SV_Target
{
