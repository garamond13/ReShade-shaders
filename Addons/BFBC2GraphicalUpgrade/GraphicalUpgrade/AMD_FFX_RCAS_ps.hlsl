// Modified AMD FidelityFX Robust Contrast Adaptive Sharpening
//
// Instead of simplified luma it uses exact luma.
//
...
float get_luma_x2(float3 color)
{
	return dot(color, float3(0.2126, 0.7152, 0.0722) * 2.0);
}
...
float4 main(float4 pos : SV_Position) : SV_Target
{
	...
	// Luma times 2.
	float bL = get_luma_x2(b);
	float dL = get_luma_x2(d);
	float eL = get_luma_x2(e.rgb);
	float fL = get_luma_x2(f);
	float hL = get_luma_x2(h);
