...
// Originaly "sharpness" is unintuitively defined as "sharpness = exp2(-sharpness)".
// We use linear range [0, 1].
#ifndef SHARPNESS
#define SHARPNESS 0.87
#endif
...
{
	float b = asfloat(0x7ef19fffu - asuint(value));
...
float4 main(float4 pos : SV_Position, noperspective float2 texcoord : TEXCOORD) : SV_Target
{
	...
	//  d e f
	//    h
	float3 b = tex.SampleLevel(smp, texcoord, 0.0, int2(0, -1)).rgb;
	float3 d = tex.SampleLevel(smp, texcoord, 0.0, int2(-1, 0)).rgb;
	float3 e = tex.SampleLevel(smp, texcoord, 0.0).rgb;
	float3 f = tex.SampleLevel(smp, texcoord, 0.0, int2(1, 0)).rgb;
	float3 h = tex.SampleLevel(smp, texcoord, 0.0, int2(0, 1)).rgb;
	...
	nz = saturate(abs(nz) * ApproximateReciprocalMedium(max3(max3(bL, dL, eL), fL, hL) - min3(min3(bL, dL, eL), fL, hL)));
	nz = -0.5 * nz + 1.0;
	...
	return float4((lobe * b + lobe * d + lobe * h + lobe * f + e) * rcpL, 1.0);
}