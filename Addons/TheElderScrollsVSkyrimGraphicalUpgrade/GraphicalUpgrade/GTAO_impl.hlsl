// XeGTAO implementation adopted for The Elder Scrolls V: Skyrim.
//
...
#define NDC_TO_VIEW_MUL (cb2[0].xy * cb2[2].xy * float2(-1.0, 1.0))
#define NDC_TO_VIEW_ADD (cb2[0].zw * float2(-1.0, 1.0))
...
void XeGTAO_PrefilterDepths16x16(uint2 dispatchThreadID, uint2 groupThreadID, Texture2D sourceNDCDepth, SamplerState depthSampler, RWTexture2D<float> outDepth0, RWTexture2D<float> outDepth1, RWTexture2D<float> outDepth2, RWTexture2D<float> outDepth3, RWTexture2D<float> outDepth4)
{
	...
	// Depth is already linearized and clamped.
	// Dividing by 100 makes AO look (right) close to the original SSAO.
	float depth0 = depths4.w / 100.0;
	float depth1 = depths4.z / 100.0;
	float depth2 = depths4.x / 100.0;
	float depth3 = depths4.y / 100.0;
...
#if XE_GTAO_FINAL_APPLY
		// Originally SSAO outputs same AO value to all 4 channels.
...
{
	// tex0 = depth
	// smp0 = g_samplerPointClamp

	XeGTAO_PrefilterDepths16x16(dtid, gtid, tex0, smp0, out_working_depth_mip0, out_working_depth_mip1, out_working_depth_mip2, out_working_depth_mip3, out_working_depth_mip4);
}
...
void main_pass_cs(uint2 dtid : SV_DispatchThreadID)
{
	...
	XeGTAO_MainPass(dtid, SpatioTemporalNoise(dtid, gtao_temporal_index), r9.xyz, tex0, smp0, ao_term_and_edges);
}
...
void denoise_pass_cs(uint2 dtid : SV_DispatchThreadID)
{
	...
	XeGTAO_Denoise(pix_coord_base, tex0, smp0, final_output);
}