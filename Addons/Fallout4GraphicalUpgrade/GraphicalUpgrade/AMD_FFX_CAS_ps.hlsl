...
float4 main(float4 pos : SV_Position) : SV_Target
{
	...
	// d e f
	// g h i
	float3 sampleA = tex0.Load(int3(pos.xy, 0), int2(-1, -1)).rgb;
	float3 sampleB = tex0.Load(int3(pos.xy, 0), int2(0, -1)).rgb;
	float3 sampleC = tex0.Load(int3(pos.xy, 0), int2(1, -1)).rgb;
	float3 sampleD = tex0.Load(int3(pos.xy, 0), int2(-1, 0)).rgb;
	float4 sampleE = tex0.Load(int3(pos.xy, 0));
	float3 sampleF = tex0.Load(int3(pos.xy, 0), int2(1, 0)).rgb;
	float3 sampleG = tex0.Load(int3(pos.xy, 0), int2(-1, 1)).rgb;
	float3 sampleH = tex0.Load(int3(pos.xy, 0), int2(0, 1)).rgb;
	float3 sampleI = tex0.Load(int3(pos.xy, 0), int2(1, 1)).rgb;
	...
	return float4(linear_to_srgb(sampleE.rgb), sampleE.a);
}