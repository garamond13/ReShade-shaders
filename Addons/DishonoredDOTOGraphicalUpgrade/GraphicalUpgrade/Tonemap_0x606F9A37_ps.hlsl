...
cbuffer PerInstanceCB : register(b2)
{
  ...
  uint cb_blackwhitemoderuntime : packoffset(c6.z);
  ...
  float cb_env_bloom_veil_strength : packoffset(c9);
}
...
Texture2D<float4> ro_postfx_lensflares_texlensflares : register(t2);
Texture2D<float4> ro_postfx_bloom_texbloom : register(t3);
Texture2D<float4> ro_viewcolormap : register(t4);
StructuredBuffer<postfx_luminance_autoexposure_t> ro_postfx_luminance_buffautoexposure : register(t5);
  ...
  out float4 o0 : SV_TARGET0)
{
  ...
  if (cb_postfx_bloom_enabled != 0) {
    ...
    r2.xyz = cb_blackwhitemoderuntime ? r1.zzz : r2.xyz;
    r1.z = cmp(0 < cb_env_bloom_veil_strength);
    if (r1.z != 0) {
      // Original bloom sampling.
      // r3.xyz = ro_postfx_bloom_texbloom.SampleLevel(smp_linearclamp_s, r1.xy, 0).xyz;

      // Bicubic bloom sampling.
      r3.xyz = sample_bicubic(ro_postfx_bloom_texbloom, smp_linearclamp_s, r1.xy).xyz;
      ...
      r3.xyz = cb_blackwhitemoderuntime ? r1.zzz : r3.xyz;
      ...
      r0.xyz = r3.xyz * BLOOM_INTENSITY + r0.xyz; // At this point r3.xyz is combind bloom and lens dirt.
    }
    ...
    #else
    r2.xyz = r1.zzz * cb_postfx_lensflares_streakopacity;
    #endif
    ...
    r1.xyz = cb_blackwhitemoderuntime ? r0.www : r1.xyz;
  ...
  r2.z = r0.x / r0.y;
  r0.xyz = saturate(cb_env_tonemapping_gamma_brightness.yyy * r2.xyz); // cb_env_tonemapping_gamma_brightness.y is allways 1?
  r0.xyz = log2(r0.xyz);
  r0.xyz = cb_env_tonemapping_gamma_brightness.xxx * r0.xyz; // At in game setting Brightness of 17 cb_env_tonemapping_gamma_brightness.x is 1.
  o0.xyz = exp2(r0.xyz);
  o0.w = 1;
  return;
}