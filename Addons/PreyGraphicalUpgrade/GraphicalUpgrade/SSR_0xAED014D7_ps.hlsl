...
Texture2D<float> reflectionDepthTex : register(t0);
Texture2D<float4> reflectionNormalsTex : register(t1);
Texture2D<float4> reflectionSpecularTex : register(t2);
Texture2D<float4> reflectionDepthScaledTex : register(t3);
Texture2D<float4> reflectionPreviousSceneTex : register(t4);
Texture2D<float2> reflectionLuminanceTex : register(t5);

#ifndef MAX_REFL_DIS_MUL
#define MAX_REFL_DIS_MUL 4.0
#endif

#ifndef NUM_SAMPLES_MUL
#define NUM_SAMPLES_MUL 4.0
#endif

// 3Dmigoto declarations
#define cmp -

// Source for most of fixes and improvements: https://github.com/Filoppi/Luma-Framework/blob/326591707e224a41d4ad290af806c9c6d97cceb9/Shaders/Prey/DeferredShading_SSR_Raytrace_0xAED014D7.ps_5_0.hlsl
  ...
  out float4 o0 : SV_Target0)
{
  ...
  // Rescaled maxReflDist
  r0.z = 1.5 * MAX_REFL_DIS_MUL * r0.z;
  ...
  // Rescaled numSamples
  r0.x = r0.x * 28.0 * NUM_SAMPLES_MUL + 4.0 * NUM_SAMPLES_MUL;
  ...
  while (true) {
    ...
    r6.zw = min(CV_HPosClamp.xy, r6.zw);
