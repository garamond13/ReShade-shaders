...
extern "C" __declspec(dllexport) const char* NAME = "TEMPLATEGraphicalUpgrade";
...
static bool on_draw(reshade::api::command_list* cmd_list, uint32_t vertex_count, uint32_t instance_count, uint32_t first_vertex, uint32_t first_instance)
{
	...
	size = sizeof(hash);
	hr = ps->GetPrivateData(g_xx_template_0x14345D29.guid, &size, &hash);
		...
		if (subobjects[i].type == reshade::api::pipeline_subobject_type::pixel_shader) {
			...
			switch (hash) {
				case g_xx_template_0x14345D29.hash:
					ensure(((ID3D10PixelShader*)pipeline.handle)->SetPrivateData(g_xx_template_0x14345D29.guid, sizeof(g_xx_template_0x14345D29.hash), &g_xx_template_0x14345D29.hash), >= 0);
					return;
			}
		}
	}
...
{
	auto buffer = (ID3D10Buffer*)resource.handle;
	D3D10_BUFFER_DESC desc;
	buffer->GetDesc(&desc);
	if (desc.BindFlags == D3D10_BIND_CONSTANT_BUFFER) {
...
static bool on_create_sampler(reshade::api::device* device, reshade::api::sampler_desc& desc)
{
	if (desc.filter == reshade::api::filter_mode::anisotropic) {
		#if DEV
		log_debug("MaxAnisotropy: {}", desc.max_anisotropy);
		#endif
